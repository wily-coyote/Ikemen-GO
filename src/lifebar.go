...
func (co *LifeBarCombo) draw(layerno int16, f []*Fnt, side int) {
	...
	a := (( float32(co.shaketime) * co.counter_mult ) * 10) * -(float32(side) * 2 - 1)
	...
	if co.text.font[0] >= 0 && int(co.text.font[0]) < len(f) && f[co.text.font[0]] != nil {
		...
		for k, v := range strings.Split(text, "\\n") {
			...
			co.text.lay.DrawText(x+sys.lifebarOffsetX + a, float32(co.pos[1])+
	...
	if co.counter.font[0] >= 0 && int(co.counter.font[0]) < len(f) && f[co.counter.font[0]] != nil {
		...
		// (x-length+sys.lifebarOffsetX)/z

		co.counter.lay.DrawText( (x-length+sys.lifebarOffsetX)/z + a, float32(co.pos[1])/z, z*sys.lifebarScale, layerno,
